...
void listDir(char *path) { //main函数的argv[1] char * 作为 所需要遍历的路径 传参数给listDir
	...
	if((log_fp = get_logfp())) fprintf(log_fp,"Scaning: %s\n", path);
	pDir=opendir(path); //  opendir方法打开path目录，并将地址付给pDir指针
	memset(childpath,0,sizeof(childpath)); //将字符数组childpath的数组元素全部置零

	if(NULL==pDir) {
		//#if DEBUG
		fprintf(stderr,"ERROR: Sorry!unknow error occurs: PATH : \"%s\" cannot be opened by \"opendir function\" or does not exist. \n", path);
		//#endif
