...
int cmpfile(char *rel_path1 ,char *rel_path2,int64_t size) {
	...
	sprintf(path1,"%s%s",get_WORKPATH(),rel_path1);
	sprintf(path2,"%s%s",get_WORKPATH(),rel_path2);
#if DEBUG
	_dbg(path1)
	_dbg(path2)
	_dbg(rel_path1)
	...
	if((ifp1==NULL)||(ifp2==NULL)) {
		...
		if((log_fp = get_logfp())) fprintf(log_fp,"Error: file cannot be opened.\n");
		exit(1);
		path1[0]='\0';
		path2[0]='\0';
	}
	...
	if((size>=0)&&(size<((int64_t)1<<20)*50L)) { //  50M>filesize>0M
		while ((fread(line1, sizeof(char), MAXSIZE, ifp1) != 0) &&
		        (fread(line2, sizeof(char), MAXSIZE, ifp2) != 0)) {
	...
	if((feof(ifp1)!=0)&&(feof(ifp2)!=0)) { // file is end
		fprintf(stderr, "Error: %s and %s cannot be read!\n",path1,path2);
		if((log_fp = get_logfp())) fprintf(log_fp,"Error: %s and %s cannot be read!\n",path1,path2);
		exit(1);
		path1[0]='\0';
		path2[0]='\0';
	}
